 ...
 */
export function preloadLuaScripts(luaDir: string): void {
  const scripts = [
    'balances.lua',
    'spendable_utxos.lua',
    'multicall.lua',
 ...
 */
export function saveScript(content: string): string {
  const hash = sha256(content);
  scriptStore.set(hash, content);
  return hash;
}
...
export class LuaRuntime {
  ...
  private constructor(factory: LuaFactory, rpcHandler: RpcHandler) {
    this.factory = factory;
    this.rpcHandler = rpcHandler;
  }

  static async create(rpcHandler: RpcHandler): Promise<LuaRuntime> {
  ...
  async executeScript(
    ...
    args: unknown[],
  ): Promise<{ calls: number; returns: unknown; runtime: number; error?: string }> {
    ...
    try {
      ...
      engine.global.set('__call_rpc', (method: string, argsJson: string): string => {
        callCount++;
 ...
 */
function luaToJs(value: unknown): unknown {
