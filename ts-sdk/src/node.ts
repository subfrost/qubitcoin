...
import type { QubitcoinDevnet } from './wasm/qubitcoin_web_sys.js';
...
export class QubitcoinNode {
  private devnet: QubitcoinDevnet;

  private constructor(devnet: QubitcoinDevnet) {
    this.devnet = devnet;
  }
   ...
   */
  static async create(config?: NodeConfig): Promise<QubitcoinNode> {
  ...
  getSpendableOutput(): SpendableOutput | null {
    return this.devnet.getSpendableOutput() as SpendableOutput | null;
  }
  ...
  getUtxoValue(txid: Uint8Array, vout: number): number | null {
    return this.devnet.getUtxoValue(txid, vout) as number | null;
  }
  ...
  static buildP2pkhScript(pubkeyHash: Uint8Array): Uint8Array {
    ...
    const wasm = (globalThis as any).__qubitcoin_wasm;
    if (wasm) return wasm.QubitcoinDevnet.buildP2pkhScript(pubkeyHash);
  ...
  static pubkeyFromSecret(secretKey: Uint8Array): Uint8Array {
    const wasm = (globalThis as any).__qubitcoin_wasm;
    if (wasm) return wasm.QubitcoinDevnet.pubkeyFromSecret(secretKey);
    throw new Error('Call QubitcoinNode.create() before using static helpers');
  }
  ...
  static hash160(data: Uint8Array): Uint8Array {
    const wasm = (globalThis as any).__qubitcoin_wasm;
    if (wasm) return wasm.QubitcoinDevnet.hash160(data);
    throw new Error('Call QubitcoinNode.create() before using static helpers');
  }
