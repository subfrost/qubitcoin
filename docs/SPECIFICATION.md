...
- Script builders: `build_p2pkh()`, `build_p2wpkh()`, `build_p2wsh()`, `build_p2sh()`, `build_p2tr()`, `build_p2mr()`, `build_op_return()`
- Script classifiers: `is_p2pkh()`, `is_p2sh()`, `is_p2wpkh()`, `is_p2wsh()`, `is_p2tr()`, `is_p2mr()`
...
| `params` | ConsensusParams for all networks (incl. P2MR and Q-Day seize heights) |
| `qday_seize` | Q-Day frozen outpoint set, seize P2MR address, commitment hash |
| `seize` | Seize transaction construction and validation |
...
- `ConsensusParams` — per-network activation heights (incl. `p2mr_height`, `qday_seize_height`), PoW limits, timing
- `FrozenOutpoints` — set of frozen quantum-vulnerable outpoints for Q-Day seize
...
- `load_frozen_set()` — load and validate embedded frozen outpoint set
- `seize_script_pubkey()` — build P2MR scriptPubKey for seize output
- `create_seize_transactions()` — construct seize txs from frozen UTXOs
- `validate_seize_transaction()` — validate a single seize transaction
- `validate_seize_block()` — validate block contains required seize txs at activation height
...
- `SEIZE_TX_VERSION = 3` — version for Q-Day seize transactions
...
### 1.16 qubitcoin-frost

**Path:** `crates/qubitcoin-frost/`
**Implements:** FROST threshold Schnorr keygen and P2MR address computation

| Module | Description |
|--------|-------------|
| `keygen` | FROST dealer-based key generation (t-of-n threshold Schnorr) |
| `address` | P2MR witness program computation from FROST group key |
| `keystore` | JSON serialization and persistence of FROST key shares |

**Key types:**
- `KeygenError` — FROST key generation error wrapper
- `FrostKeystore` — serializable keystore entry for a single signer

**Key functions:**
- `generate_frost_keys(signers, threshold)` — generate FROST key shares via trusted dealer protocol
- `group_verifying_key(pkg)` — extract 32-byte x-only group verifying key
- `frost_group_key_to_p2mr_program(pkg)` — compute P2MR witness program (tapleaf hash of FROST group key check script)
- `build_p2mr_frost_script(program)` — build P2MR scriptPubKey from witness program
- `save_all_shares(shares, pubkey_pkg, dir)` — persist all signer key shares as JSON files

**Dependencies:** `frost-secp256k1-tr` (FROST protocol), `frost-core`

---

### 1.17 qday-regression

**Path:** `patch/qday/regression/`
**Implements:** Q-Day freeze mechanism — scores and freezes quantum-vulnerable UTXOs

| Module | Description |
|--------|-------------|
| `consensus` | `check_qday_frozen()` enforcement in connect_block |
| `error` | Q-Day error types |
| `frozen_set` | `FrozenOutpoints` HashSet with serialization and commitment hash |
| `policy` | `QdayPolicy` configuration (activation height, score threshold) |
| `scanner` | UTXO scanning and frozen set computation |
| `scoring` | Vulnerability scoring (age, dormancy, pattern, value, pubkey exposure) |

**Key types:**
- `QdayPolicy` — activation height + score threshold + frozen set
- `FrozenOutpoints` — O(1) membership test for frozen outpoints
- `VulnType` — vulnerability classification (P2PK, P2PKH, P2TR exposed keys)

**Key functions:**
- `check_qday_frozen(tx, policy, height)` — reject transactions spending frozen outpoints
- `compute_frozen_set(utxo_iter, exposed_keys, threshold)` — build frozen set from UTXO scan
- `compute_score(entry, exposed_keys)` — score a UTXO for quantum vulnerability
...
| Constant | Value | Description | Source |
|----------|-------|-------------|--------|
| `P2MR_CONTROL_BASE_SIZE` | 1 | Control block base: leaf version only (no internal key) | `crates/qubitcoin-script/src/interpreter.rs` |
| `P2MR_CONTROL_NODE_SIZE` | 32 | Merkle path node size | `crates/qubitcoin-script/src/interpreter.rs` |
| `P2MR_CONTROL_MAX_NODE_COUNT` | 128 | Maximum merkle path depth | `crates/qubitcoin-script/src/interpreter.rs` |
| `SEIZE_TX_VERSION` | 3 | Transaction version for Q-Day seize transactions | `crates/qubitcoin-consensus/src/seize.rs` |
| `MAX_SEIZE_INPUTS_PER_TX` | 5,000 | Maximum inputs per seize transaction | `crates/qubitcoin-consensus/src/seize.rs` |
...
| Taproot (BIP341) | 709,632 | `crates/qubitcoin-consensus/src/params.rs` |
| P2MR (BIP 360) | i32::MAX (TBD) | `crates/qubitcoin-consensus/src/params.rs` |
| Q-Day Seize | i32::MAX (TBD) | `crates/qubitcoin-consensus/src/params.rs` |

**Note:** P2MR and Q-Day Seize are active from height 0 on regtest and height 1 on testnet/signet. Mainnet activation heights are `i32::MAX` (placeholder, to be determined with community support).
...
| 360 | P2MR (Pay-to-Merkle-Root) | TBD (i32::MAX) | script, consensus, node, frost | `verify_p2mr_commitment()`, `build_p2mr()`, witness v2 dispatch | P2MR unit tests |
| — | Q-Day Seize | TBD (i32::MAX) | consensus, node | `create_seize_transactions()`, `validate_seize_block()`, `connect_block()` seize enforcement | Seize unit tests |
...
connect_block()                   crates/qubitcoin-node/src/validation.rs:1024
  ...
  ├── Q-Day seize enforcement (at qday_seize_height):
  │     ├── load_frozen_set()
  │     ├── create_seize_transactions()
  │     └── validate_seize_block() — block MUST contain seize txs
  ├── check_inputs() — verify UTXOs exist
  ├── get_block_script_flags() (includes P2MR flag)
...
verify_script()                   crates/qubitcoin-script/src/interpreter.rs:1638
  ...
  │     └── script-spend:
  │           ├── verify control block (33 + N*32 bytes)
  │           ├── verify_taproot_commitment() (internal key tweak)
  │           ├── OP_SUCCESS pre-scan
  │           └── eval tapscript
  ├── if witness v2 (P2MR / BIP 360):
  │     └── script-spend only (no key-spend):
  │           ├── annex detection (0x50 tag)
  │           ├── validate control block (1 + N*32 bytes, no internal key)
  │           ├── verify_p2mr_commitment() (merkle root == program, NO key tweak)
  │           ├── OP_SUCCESS pre-scan
  │           └── eval tapscript (leaf version 0xc0)
  └── CLEANSTACK check
```
...
| P2MR | `OP_2 <32-byte-merkle-root>` | BIP 360 script-path only, witness: `[args..., script, control_block]` |
...
| qubitcoin-script | 104 | Opcode encoding, ScriptNum, interpreter, verify_script, P2MR flag | 1,217 script vectors (`script_tests.json`) |
| qubitcoin-consensus | 96 | Transaction validation, merkle, sighash, block checks, seize tx, frozen set | 214 tx vectors (`tx_valid.json`, `tx_invalid.json`) |
...
| qubitcoin-net | 161 | Protocol serialization, message handling, peer management | — |
| qubitcoin-rpc | 104 | JSON-RPC parsing, method dispatch, response formatting | — |
...
| qubitcoin-frost | 7 | FROST 170/255 keygen, P2MR address, keystore roundtrip | — |
| qday-regression | 26 | Frozen set, commitment hash, scoring, consensus enforcement | — |
| **Total** | **1,058+** | | **1,431 external vectors** |
...
## 10. P2MR (BIP 360) and Q-Day Seize Architecture

### P2MR: Pay-to-Merkle-Root (SegWit Version 2)

P2MR is a new output type for quantum-safe custody. Key design differences from taproot (P2TR):

| Property | P2TR (v1) | P2MR (v2) |
|----------|-----------|-----------|
| Witness version | 1 (`OP_1`) | 2 (`OP_2`) |
| Key-path spend | Yes (Schnorr sig on output key) | **No** — script-path only |
| Internal key in control block | Yes (32 bytes at `control[1..33]`) | **No** — control block starts with merkle nodes |
| Control block base size | 33 bytes (version + internal key) | **1 byte** (version only) |
| Commitment verification | `tap_tweak(internal_key, merkle_root) == output_key` | `merkle_root == witness_program` (no tweak) |
| Public key exposure | Output key is on-chain | **No key on-chain** — only merkle root |

**Why no key-path?** P2MR is designed for quantum safety. Exposing a public key on-chain (even an x-only key) creates a quantum vulnerability. By using only the merkle root of a script tree, no public key is ever revealed until spend time.

### P2MR Witness Stack Layout

```
witness: [arg1, arg2, ..., script, control_block]
```

- **Control block:** `[leaf_version (1 byte)] [merkle_node (32 bytes)]*`
- **Verification:** Compute tapleaf hash from script, walk merkle path, compare root to witness program directly
- **Script execution:** If leaf version is 0xc0, execute as tapscript (BIP 342 rules)

### FROST 170/255 Multisig

The FROST threshold Schnorr protocol generates a group key that can produce valid Schnorr signatures with any 170 of 255 signers, without revealing individual key shares.

```
FROST Keygen (trusted dealer)
  └── generate_with_dealer(255, 170)
        ├── 255 KeyPackage shares (one per signer)
        └── PublicKeyPackage (group verifying key)

Group Key → P2MR Address
  ├── Extract x-only group key (32 bytes)
  ├── Build leaf script: <xonly_key> OP_CHECKSIG
  ├── Compute tapleaf hash: tagged_hash("TapLeaf", [0xc0, compact_size(script), script])
  └── Tapleaf hash IS the P2MR witness program (merkle root)
```

### Q-Day Seize Mechanism

At the seize activation height, quantum-vulnerable UTXOs are consensus-seized:

```
1. Frozen set: Computed offline by metashrew analyzer + qday-regression scoring
   └── Embedded as FROZEN_OUTPOINTS_DATA constant in qday_seize.rs

2. FROST address: Generated via FROST 170/255 DKG ceremony
   └── Embedded as SEIZE_P2MR_PROGRAM constant in qday_seize.rs

3. At qday_seize_height:
   ├── load_frozen_set() — deserialize and validate commitment hash
   ├── create_seize_transactions() — batch frozen UTXOs into v3 txs
   │     ├── Each tx: version=3, frozen inputs, single P2MR output
   │     └── Batched at MAX_SEIZE_INPUTS_PER_TX (5000) inputs per tx
   └── validate_seize_block() — block MUST contain exact seize txs after coinbase

4. After seize height:
   └── Frozen outpoints no longer exist in UTXO set (already seized)
```

### Seize Transaction Format

| Field | Value | Description |
|-------|-------|-------------|
| version | 3 | Distinguishes from regular transactions |
| vin | frozen outpoints | Empty scriptsig/witness (consensus-validated) |
| vout | 1 output | P2MR to FROST group key, value = sum(inputs) |
| lock_time | 0 | No lock time |

### Activation Heights

| Network | `p2mr_height` | `qday_seize_height` |
|---------|---------------|---------------------|
| Mainnet | `i32::MAX` (TBD) | `i32::MAX` (TBD) |
| Testnet3 | 1 | 1 |
| Signet | 1 | 1 |
| Testnet4 | 1 | 1 |
| Regtest | 0 (always active) | 0 (always active) |
